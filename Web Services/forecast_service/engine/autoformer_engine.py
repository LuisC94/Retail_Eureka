...
def train_model(df_data: pd.DataFrame) -> bytes:
    """
    Treina o modelo Autoformer e devolve os bytes do artefacto.
    """
    set_seed(CONFIG["seed"])

    if "date" in df_data.columns and "Data" not in df_data.columns:
        df_data = df_data.rename(columns={"date": "Data"})
    if "sales_quantity_kg" in df_data.columns and "Valor" not in df_data.columns:
        df_data = df_data.rename(columns={"sales_quantity_kg": "Valor"})
...
def predict_horizon(model_bytes: bytes, running_history: list, avg_price: float, horizon_days: int = 30) -> list:
    """
    Executa inferência com o modelo Autoformer gravado.
    """
