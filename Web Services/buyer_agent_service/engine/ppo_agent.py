...
class ParallelRolloutBuffer:
    """ Buffer adaptado para N-ambientes em paralelo """
    def __init__(self):
        self.states = []
        self.actions = []
        self.logprobs = []
...
class ParallelPPOAgent:
    def __init__(self, state_dim=17, action_dim=1, max_action=500, lr_actor=0.0003, lr_critic=0.001, gamma=0.8, K_epochs=30, eps_clip=0.2, batch_size=1024):
    ...
    def save(self, target_dir):
        ...
        torch.save(self.policy_old_actor.state_dict(), os.path.join(target_dir, 'actor.pth'))
        torch.save(self.policy_old_critic.state_dict(), os.path.join(target_dir, 'critic.pth'))
        scaler_state = {
            'n': self.reward_scaler.n,
            'mean': self.reward_scaler.mean,
            'S': self.reward_scaler.S
        }
        torch.save(scaler_state, os.path.join(target_dir, 'scaler.pth'))
        
    def load(self, target_dir):
        actor_path = os.path.join(target_dir, 'actor.pth')
        critic_path = os.path.join(target_dir, 'critic.pth')
        scaler_path = os.path.join(target_dir, 'scaler.pth')
        
        if os.path.exists(actor_path) and os.path.exists(critic_path):
            self.policy_actor.load_state_dict(torch.load(actor_path, map_location=self.device, weights_only=False))
            self.policy_critic.load_state_dict(torch.load(critic_path, map_location=self.device, weights_only=False))
            self.policy_old_actor.load_state_dict(self.policy_actor.state_dict())
            self.policy_old_critic.load_state_dict(self.policy_critic.state_dict())
            
            if os.path.exists(scaler_path):
                scaler_state = torch.load(scaler_path, map_location='cpu', weights_only=False)
                self.reward_scaler.n = scaler_state.get('n', 0)
                self.reward_scaler.mean = scaler_state.get('mean', np.zeros(()))
                self.reward_scaler.S = scaler_state.get('S', np.zeros(()))
            return True
        return False
