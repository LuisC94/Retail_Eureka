...
class StockEnvironment:
    ...
    def __init__(self, excel_path, is_training=True, train_split=0.6, max_capacity=1000, shared_stats=None,
        ...
        if is_training:
            ...
            print(f"Environment Constrained initialized in TRAINING Mode: {len(self.data)} days. Max Order Limit: {self.max_order_limit}")
        else:
            self.data = self.df.iloc[split_index:].reset_index(drop=True)
            print(f"Environment Constrained initialized in TESTING Mode: {len(self.data)} days. Max Order Limit: {self.max_order_limit}")
            ...
            if apps.ready:
                ...
                if subfamily and subfamily.lifecycle_presets:
                    ...
                    for k, v in subfamily.lifecycle_presets.items():
                        merged_p[k] = v
                    self.PRESETS[self.fruit_key] = merged_p
                    print(f"[StockEnv] Coeficientes biológicos de {self.fruit_key} carregados com sucesso da base de dados.")
        except Exception:
            pass
    ...
    def step(self, action_quantity, update_stats=True):
        ...
        raw_order = max(0, min(action_quantity, self.max_order_limit, self.max_capacity - current_total_stock))
